	...
	def __init__(self):
        ...
        def _build_dynamic_model(self, is_annotated, device):
                ...
                for i in xrange(NUM_DATA):
                    ...
                    inputs.append(tf.placeholder(tf.float32, [None, NUM_CLASSES]))
                state_outputs = []
        ...
        #def _build_phi_s_model(self, l_input):
                ...
                #return out_state
        def _build_phi_s_model(self, l_input):
            #return learn.DNNClassifier(hidden_units=[128,64])
                l_dense = tf.layers.dense(l_input, 128, activation=tf.nn.elu,name='phi_s_dense')
                l_dense1 = tf.layers.dense(l_dense, 64, activation=tf.nn.elu,name='phi_s_dense1')
                out_state = tf.layers.dense(l_dense1, STATE_SIZE, activation=tf.nn.elu,name='phi_s_out_state')
                return out_state
        ...
        def _build_pi_model(self, p_dist_i, phi_s):
                ...
                l_dense1 = tf.layers.dense(phi_s, 32, scope='dense1_pi_')
                l_dense2 = tf.layers.dense(p_dist_i, 16, activation_fn=tf.nn.elu,name='dense2_pi_')
                l_concat = tf.concat([l_dense1, l_dense2], axis=1, name='concat_pi_model')
                out_action = fully_connected(l_concat, 1, activation=tf.nn.linear, name='out_action_')
