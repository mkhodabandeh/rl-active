        ...
        with self.sess.as_default():
            with self.graph.as_default() as g:
                with tf.device(self.device):
                    ...
                    arr = np.zeros((data.shape[0], self.num_classes))
                    # print arr.shape
                    batch_size = 128
                    for i in xrange(data.shape[0]/batch_size):
                        # arr[i*batch_size:(i+1)*batch_size,...] =  
                        temp = np.array(self.model.predict({'lenet_input': data[i*batch_size:(i+1)*batch_size, ...]}))
                        # print temp.shape
                        ...
                        # print temp[0:2, ...]
                        # pdb.set_trace()
                    # print (i+1)*batch_size, data.shape[0]
                    i = data.shape[0]/batch_size
                    if (i)*batch_size != data.shape[0]:
                        # print 'hi'
                        temp = np.array(self.model.predict({'lenet_input': data[(i)*batch_size:, ...]}))
                        arr[(i)*batch_size:, ...] = temp

                    # return np.array(self.model.predict({'lenet_input': data}))
                    return arr

    def _predict_old(self, data=None):
        print('[lenet_tflearn]Doing Predictions...')
        with self.sess.as_default():
            with self.graph.as_default() as g:
                with tf.device(self.device):
...
def test_lenet(num_data, init_n, sample_n, gpu_n, myid, log_path):
    ...
    lenet.train()
    init_acc = lenet.evaluate()
    print init_acc
    data = lenet.x_train[range(num_data/2, num_data)]
    preds = lenet._predict(data=data)
    from scipy.special import entr
    entropy = entr(preds).sum(axis=1)/np.log(2)
    print 'entorpy.shape',entropy.shape
    indices = range(num_data/2, num_data)
    
    entropy_choices = entropy.argsort()[-sample_n:]
    print 'entropy_high', entropy[entropy_choices]
    entropy_choices = entropy.argsort()[:sample_n]
    print 'entropy_high', entropy[entropy_choices]
    print entropy_choices.shape 
    lenet._reset()
    lenet.set_set_annotations(entropy_choices.tolist()+first_set)
    lenet.train()
    acc = lenet.evaluate()
    print len(range(num_data/2, num_data))
    print acc
    exit()
